...
- 函数式编程
  - 函数式编程是一种编程范式，其核心思想是：强调使用**纯函数**来构建软件，避免可变状态和副作用
  - 特征
    - **纯函数**：函数式编程中的函数指的是**数学**中的函数，即一个输入到输出的映射，其特点：
      - 给定同样的输入有且只有唯一的输出
        - 意味着函数的结果只能依赖于输入参数
      - **不可变数据**：变量一旦初始化之后就不再改变，也就是**没有赋值**。函数式编程语言中的变量不是命令式编程语言中的变量，即存储状态的单元，而是代数中的变量，即**某个符号所对应的值是永远不变的**
        - 如何
      - **无副作用**：函数不会产生除计算结果以外的事，比如 IO 操作、全局状态修改等
    - **函数是一等公民**：函数能够赋值给变量，可以作为参数传递，也可以作为返回值返回
    - 强调使用**递归用于代替循环**，**高阶函数**用于操作**组合**其他函数
