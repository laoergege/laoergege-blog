...
- 设计模式
  ...
  - [ ] 你项目里面都用了哪些设计模式
- js 执行机制
  - 变量提升、var、let、const 区别及暂时性死区
    - 变量提升其实就是声明提升，相当于代码执行之前 JavaScript 引擎把变量的声明的部分提升到代码开头，实际上变量在源码里的位置是不会改变的，而是在编译期间，JavaScript 引擎会先将变量、函数想在内存中创建。
    - 作用域
      - 全局
      - 模块
      - 函数
...
- 异步、事件循环
  ...
  - 递归 setTimeout: 模拟 setInterval
    ...
    ```js
    const setInterval2 = (func, time, ...params) => {
        const timer = () => {
            func(...params)
  ...
  - 实现并发请求限制
    ```js
    const concurry = async (tasks, maxNum) => {
      const queue = []
...
- webwork
  - Excel 大文件导出
    - 在 worker 线程中通过 exceljs 构建表格相关数据
    - file-save
- css
