...
- js
  ...
  - 模块
    ...
    - [x] 为什么模块循环依赖不会导致死循环？CommonJS和ES Module的解决办法有什么不同？
  ...
  - js 执行机制
    ...
    - [x] 变量提升、var、let、const 区别及暂时性死区
      ...
      - *所有的声明（function、var、let、const 和 class）都在 JavaScript 编译期间被提升*
      - var
        - 全局或者函数级别作用域
        - 可重复声明、重复赋值
      - let、const
  ...
  - JS 内存管理机制
    ...
    - [x] v8 引擎垃圾回收策略 
      ...
      - 新生代使用 Scavenge (清道夫)算法
        4. 将内存分为两个对等空间，分别为 From 区域和 To 区域。 
        5. 新加入的对象都会存放到 From 区域
        6. 当 From 区域快被写满时执行清除操作：跟踪 From 区域的活动对象并复制到 To 区域，相当于完成了内存整理操作，复制后空闲区域就没有内存碎片了
        7. 完成复制后，From 区域与 To 区域进行角色翻转
        8. 对象晋升策略：经过两次垃圾回收依然还存活的对象，会被移动到老生区中*
      - 老生代使用Mark-Sweep-Compact算法完成
        9. 标记：从一组 GC Root 对象开始进行**可达性分析**，标记活动对象和非活动对象
        10. 清除：清除非活动对象，并将碎片空间记录到空闲列表以便复用  
        11. 整理：当高度碎片化或者空间不足时进行内存整理，将所有存活的对象移动到一端的空闲位置
    - [ ] 内存泄漏及优化
...
- web
  ...
  - [x] 安全
    ...
    - Cookie中的「同站」判断就比较宽松：只要两个 URL 的 协议、eTLD（有效顶级域） + 1 相同即可
  ...
  - [x] async 或 defer
    - 都是异步加载 js
    - async 加载完立刻执行
    - defer 在 DOM（DOMContentLoaded） 解析完毕再执行，并且如果有多个defer脚本，会按照它们在页面出现的顺序加载
  - [ ] 讲讲 fetch 和 xhr 的区别
