...
- 操作系统：操作系统屏蔽硬件层面的异步。通常编程语言都是同步的，但实际上我们的程序在计算机内部却是并发执行的，操作系统以线程作为最小调度执行单位并且分时调度我们的代码（操作系统会保存上下文及执行位置以便恢复原来执行），并且（主要）提供了阻塞的系统调用，这样的一切使得我们程序表现出一种同步执行效果，对于开发者来说是无感的
- 并发模型
- 异步编程范式
