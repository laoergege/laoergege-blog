...
浏览器多进程架构中，浏览器进程使用 Blink 渲染引擎创建一个沙箱的渲染进程，渲染器进程的核心工作是将 HTML、CSS 和 JavaScript 等 Web 内容转换为用户可以与之交互的网页。

> Blink 是 Web 渲染引擎，参考文档 [how-blink-works](https://docs.google.com/document/d/1aitSOucL0VHZa9Z2vbRJSyAIsAz24kX8LFByQ5xQnUg/edit) 。
...
![](./images/Life%20of%20a%20Pixel.png)

WebContents 一般指的是构成网页的物件（文本、图片、HTML、CSS、JS），
...
渲染是一个复杂的过程，为了提高渲染效率，整个渲染渲染流程就像是一条管道，分成多个渲染阶段，每个阶段会有来自上一阶段的输入内容，和对下一阶段的输出产物。

当发生更新时，就可以重其中某一阶段开始执行并复用之前的产物，并且分多阶段任务执行，可以提高系统任务调度灵活性。

![](./images/stages.png)
## 页面渲染流程
### 构建 DOM（Parse HTML）

将 HTML 解析成树形结构的 DOM 数据。

![](./images/微信图片_20200606191641.png)
### 样式计算（Recalculate Style）

1. 解析 CSS 规则
   > 同 HTML 解析一样，将 CSS 文本解析成内部数据结构 styleSheets，在控制台中输入 document.styleSheets 查看。
2. 转换样式表中的属性值，使其标准化
   > 将多种 css 值单位标准化
   ![](./images/1252c6d3c1a51714606daa6bdad3a560.png)
3. 计算出 DOM 树中每个节点的具体样式
   > 计算出 DOM 节点中每个元素的具体样式，并被保存在 ComputedStyle 的结构内,在计算过程中需要遵守 CSS 的**继承和层叠**两个规则。
### 布局阶段（Layout）
#### 1. 创建布局树（Layout Tree）

遍历 DOM 树中的所有可见节点，并把这些节点复制加到布局树中。

![](./images/微信截图_20200606211336.png)

DOM 并不是跟 layout 的节点并不是一对一对应关系，不可见的节点包括：
...
布局阶段并没有清晰地将输入内容和输出内容区分开来

网络进程在接收到响应头时就会通知浏览器进程，然后浏览器进程收到通知就会告诉渲染进程进行提交文档的准备，这时渲染进程就和网络进程建立通道了，也就是已经开始了提交文档的流程，等文档接收、解析完毕时，渲染进程通知浏览器进程文档提交确认，最后浏览器移除原有界面，更新现在的状态
