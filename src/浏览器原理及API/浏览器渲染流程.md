...
![](./images/layer-list.png)

并不是 DOM 中的每个节点都包含一个图层，具有某些特定样式的节点才会被提成为单独的图层，如果一个节点没有对应的层，那么这个节点就从属于父节点的图层
...
#### LayerTrees（GraphicsLayers）

要能成为图层的节点必须要有成为 PaintLayer 层叠上下文属性条件。 但是并不是所有的 PaintLayer 都能成为 GraphicsLayer，**因为生成图层是会占用内存**，只有某些特殊的 PaintLayer 才会被提升为 GraphicsLayer。要形成单独图层的元素需要具有以下条件：
...
> 目前 Layers 发生 Paint（下文见）前，未来将被转移 Paint 后面 
