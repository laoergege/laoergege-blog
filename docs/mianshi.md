...
- http
  ...
  - [x] 浏览器 http 缓存
    - HTTP 缓存又分为强缓存和协商缓存
    1. 强制缓存：通过 Expires 和 Cache-Control 判断缓存是否可用，如果可用则直接使用，否则协商缓存
  ...
  - [x] TCP 连接过程：说一下三次握手四次挥手
    - 握手
      1. 客户端向服务端发送一个SYN(seq=x)报文，并指明客户端的初始化序列号，请求建立连接
      2. 服务端发送 SYN+ACK (seq=y,ack=x+1) 报文，并指明服务端端的初始化序列号给客户端
