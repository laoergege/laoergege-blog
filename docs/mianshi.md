...
- http
  ...
  - [ ] TCP 可靠性
  - [x] TCP, UDP 的区别
    - TCP 是一种面向连接、可靠的的传输层通信协议，适用于要求可靠传输的应用，例如点对点文件传输、邮件等
    - 而 UDP 则是面向用户数据报文的协议，不需要连接，不提供可靠性，只管发送报文。结构简单、灵活性高，适合扩展，比如 http3 的 QUIC 就是基于 UDP。UDP传输更快，适用于实时应用，比如音频、视频传输这种。
  - [x] CNAME 记录的作用？
