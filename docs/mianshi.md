...
- http
  ...
  - [x] https 是什么，与 http 区别
    ...
    - https 端口由原 http 默认的 80 改为 443，其他协议内容保持跟 http 一样
  - [ ] https 握手流程
  ...
  - [x] 浏览器 http 缓存
    - HTTP 缓存又分为强缓存和协商缓存
    1. 强制缓存：浏览器发送请求前先通过 Expires 和 Cache-Control 判断缓存是否可用，如果可用则直接使用，否则协商缓存
    2. 协商缓存：客户端直接发起请求并携带 If-Modified-Since 、If-None-Match 去请求后台，服务器根据条件请求字段判断资源是否更新
       - 若资源更新，返回资源和 200 状态码
  ...
  - [x] TCP 连接过程：说一下三次握手四次挥手
    - 握手
      1. 客户端向服务端发送一个SYN 同步(seq=x)报文请求建立连接，并指明客户端的初始化序列号
      2. 服务端返回 SYN+ACK (seq=y,ack=x+1) 报文，并指明服务端端的初始化序列号给客户端
      3. 客户端发送 ACK（seq=x+1,ack=y+1）确认报文给服务端
    - 挥手
      - 客户端发送 FIN
      - 服务端发送 ACK
      - 服务端发送 FIN
  ...
  - [x] 状态码
    ...
    - 3xx: 资源位置发生变动，需要客户端重定向请求资源位置
      - 300：多个重定向选择
      - 301：永久性重定向
      - 302：临时性重定向，这个资源只是暂时不能被访问了，但是之后过一段时间还是可以继续访问，一般是访问某个网站的资源需要权限时，会需要用户去登录，跳转到登录页面之后登录之后，还可以继续访问。
      - 304：资源未被修改，客户端重定向缓存
...
- 框架 vue
  - 原理
    ...
    - [x] 数据绑定原理
      ...
      - [x] vue 双向绑定原理（v-model）：数据劫持 + 事件监听
        - v-model 只是语法糖，实际会转成数据绑定 + 事件监听
...
1. 框架进阶一步去使用，比如它的一些高级用法是否有所掌握，有没有试着去了解它的原理实现
2. 日常的业务开发中不局限于完成功能，是否有去思考项目结构如何设计，如何封装基础工具，基础组件如何设计、开发、共享
3. 团队提效
