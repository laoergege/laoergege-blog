...
- 面向对象
  - 继承
    - Java，继承本质类和对象的拷贝从属关系
    - JS，原型链委托关系
    - 代码复用：我们可以把通用功能放到抽象类；而一些特定的行为或属性，我们可以通过继承放到实现类中
      - 问题：父子的层级过于复杂，也会变得“官僚化”，如果父类有了问题，就会牵一发动全身，而且抽象的层级过多，也会让代码难以理解
        - 组合：为了解决继承的层级过多的情况，在面向对象中，也有组合优于继承的思想。
    - 抽象基类 、接口区别？继承还有什么意义 
- js 编程模式
  - 事件驱动
  - 动态类型
...
- 继承
  ...
  - 继承后移
    - 重写
    - 重载
  - 继承可以复用，但不是所有复用都能用继承，继承的目的当然是为了多态
    - 继承原始目的复用
...
- 多态是静态分派和动态分派
- 代码重用：继承基类的方法，并做出自己的改变和/或扩展
- 继承
  - 实现继承
  - 接口继承
- 继承导致强耦合(后来通过设计模式的策略模式解决)
- 严重制约了多态能力(通过面向接口编程解决)
- 继承的缺点主要是很容易被滥用
  - 调用一个方法时，比较难弄清楚这个方法是这个类自己的还是继承自基类，或者说方法被覆写过行为可能不一样
- 新兴语言的基本组成都是 immutable struct + protocol/trait 了，相当于 没有继承的class + 独立在数据之外的interface的implements
...
- 值要使用
  - 拷贝/复制：如果一个类型实现了 Copy trait，那么一个旧的变量在将其赋值给其他变量后仍然可用
