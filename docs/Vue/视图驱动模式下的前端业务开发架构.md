...
在前后分离的情况下，**大多数前端业务开发流程都是以视图驱动的模式**：

1. 先根据 UI 稿子抽象成组件、面向组件写页面
2. 再根据原型文档与接口完成业务交互

组件优先：在如今以 React、Angular、Vue 为首的主流前端开发框架都是以组件为基础的开发范式。组件概念借鉴了“OOP”的封装概念，但抛弃了传统的“OOP”的继承和多态思想；react又额外借鉴了一些fp中的风格

组件 = 视图逻辑 + 状态逻辑

通过面向对象之类的思想来对界面或者业务做模块化尝试复用,
