...
- 页面加载
  - 指标
    ...
    - 首屏
    - 首屏可交互时间（FID、TBT、TTI）
  - 优化原则：尽量仅加载首屏渲染所需要的资源内容，即[关键资源](#关键资源)
    - 资源请求
      ...
      - TTFB：第一字节时间 ![](./images/timestamp-diagram.svg)
        ...
        - HTTP/1 keep-alive
        - [缓存：HTTP 缓存、CDN 代理缓存](../HTTP/http%20缓存.md)
        - 减少请求数据大小
          - HTTP/2（头部压缩）
        - 提高服务器性能
        - 网络层面
    ...
    - 延迟加载非关键资源
      ...
      - 图片懒加载
    - 提高关键请求优先级：避免请求链过长，导致整体请求时间延长
      - Resource Priority Hints
        - preload
        - prefetch
      - 资源内联：通过 HTML 内联 JavaScript、CSS 来移除这两种类型的文件下载时间
...
或者通过 DevTools Performance 面板观察 FCP、LCP 或者自定义首屏指标之前发起的请求链去进行确认优化：

1. 延迟加载非关键请求
2. 提高请求优先级：避免请求依赖链过长，导致整体请求时间延长
   - 预加载依赖
   - 资源内联、合并
...
- 动态脚本，类似 async - 显式地设置了 `script.async=false`，则可以改变这个规则。然后脚本将按照脚本在文档中的顺序执行，就像 defer 那样
