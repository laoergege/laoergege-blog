...
个人觉得这种异步调用模型算是异步编程的一种实现形式，但我更偏向广义去了解解释何为异步编程...
...
## JavaScript 的执行环境：事件循环 + 回调队列

JavaScript 是一门单线程语言，同时也跟大多数语言一样都是默认同步编程模型，一旦遇到耗时任务整个线程就会被同步阻塞。对于 JavaScript 来说，耗时任务的异步化主要由执行环境提供的异步 API，并且在异步处理上则通过异步回调的范式。可以说在编程层面，**JavaScirpt 异步 = 异步 API + 异步回调**的模式。
...
在编程语言语义中，Promise 意味着一种异步，当产生的结果是异步错误，自然不会抛出原 Promise 代码外部，本质上是异步代码不在原调用栈中执行。

但实际上 JavaScirpt Promise 的执行者（executor）是同步执行，而处理程序（handler）是异步执行，但我们可以认为 Promise 的执行者（executor）和 promise 的处理程序（handler）周围有一个“隐式的 try..catch”。如果发生异常，它就会被捕获，这样就可以保持在语义上相同了。
...
function _async(gen) {
  ...
  return new Promise((resolve, reject) => {
    const exec = function(res) {
      try {
        let { value, done } = g.next(res);
        if(done) {
          resolve(value) 
        } else {
          Promise.resolve(value).then(exec, (err) => g.throw(err))
        }
      } catch (error) {
        reject(error)
      }
...
- function* 换成 async
- yield 换成 await
- yield 的 **value 用 Promise.resolve(value) 包装**
...
- Generator
  - yield 返回一个迭代器对象，该对象有 value 和 done 两个属性


 thunk 函数，它的基本思路都是接收一定的参数，会生产出定制化的函数，最后使用定制化的函数去完成想要实现的功能。