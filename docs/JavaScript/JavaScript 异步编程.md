...
Promise 通过链式调用的写法解决了回调地狱的问题，并且保证了相关内聚写在一起，更符合人的思维模式，提高了一定的可读性。
...
可以认为 Promise 的执行者（executor）和 promise 的处理程序（handler）周围有一个“隐式的 try..catch”。如果发生异常，它就会被捕获，经过 reject 进行处理，将该 Promise 对象也变为 Rejected 状态。
