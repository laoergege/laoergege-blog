      ...
      - 自动回收策略：垃圾数据是由**垃圾回收器**来释放的，并不需要手动通过代码来释放
        ...
        - 垃圾回收算法机制
          ...
          - [标记-清除](#标记-清除算法)
            ...
            - [V8 堆内存回收机制](#v8-堆内存回收)
              - [分代收集](#新生代和老生代)
                - [新生代采用 Scavenge 算法回收](#新生代采用-scavenge-算法回收)
                - [老生代垃圾回收：标记-清除-整理算法](#老生代垃圾回收标记-清除-整理算法)
              - 回收效率优化
                - 副垃圾回收器：并行回收
                - 主垃圾回收器：并发标记、并行清理压缩、增量式回收
          - *所有权&借用*
- 内存问题
